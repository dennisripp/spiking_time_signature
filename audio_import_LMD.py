...
def load_ground_truth():
    tsv_path = os.path.join(root_path, "lmd-tempo.tsv")
    ground_truth_df = pd.read_csv(tsv_path, sep='\t', header=None, names=["filename", "bpm", "genre"])
    return ground_truth_df.set_index('filename')['bpm'].to_dict()
...
def process_directories(root_dir, id_set):
    for dirpath, dirnames, filenames in os.walk(root_dir, topdown=False):
        dir_id = os.path.basename(dirpath)
        if dir_id in id_set:
            move_directory(dirpath, root_dir)
...
def get_bpm_from_ground_truth(audio_path):
    ...
    ground_truth = ground_truth_dict.get(parent_folder)
...
def load_and_preprocess_data_subdir(args):
    ...
    filtered_files = [f for f in all_files if os.path.splitext(f)[1] == ".mid"]
...
def parallel_load_and_preprocess(directory):
    ...
    # filter everything but directories
    all_dirs = [f for f in all_dirs if os.path.isdir(os.path.join(directory, f))]
    
    print(f"Found {len(all_dirs)} dirs in {directory}")
    
    # look in every subdir and get all midi files 
    all_files = []
    for subdir in all_dirs:
        for file in os.listdir(os.path.join(directory, subdir)):
            if os.path.splitext(file)[1] == ".mid":
                all_files.append(os.path.join(directory, subdir, file))
    ...
    if len(all_files) <= 0:
        raise Exception("Not enough files in directory")
    
    files_to_load = all_files[:FILES_TO_LOAD]
    ...
    with ProcessPoolExecutor(max_workers=CORE_COUNT) as executor:
        ...
        results = list(tqdm(executor.map(load_and_preprocess_single_song, all_files), total=len(all_files)))
...
def preprocess_audio(file_path, ground_truth):
    ...
    sf = FluidSynth(soundfont_path)
    
    # get file path without extension
    file_path_w = os.path.splitext(file_path)[0]
    
    sf.midi_to_audio(file_path, f'{file_path_w}.wav')
    
    y, sr = librosa.load(f'{file_path_w}.wav', sr=SR)
...
def main():
    trainset, valset, shape = import_audio_get_loader(batch_size=32, only_dataset=True, train_data_ratio=0.8, files_to_load=20, sr=SR)

   
# def main():
#     global processed_cnt, deleted_cnt
#     tsv_path = os.path.join(root_path, 'lmd-tempo.tsv')
#     id_tempo_dict = parse_tsv(tsv_path)
#     process_directories(root_path, id_tempo_dict)
